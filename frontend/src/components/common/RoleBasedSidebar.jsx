...
import { 
  ...
  UploadCloud, Wallet, Shield, BarChart3, LogOut, ChevronUp, ChevronDown, HelpCircle, Plus, Compass
} from 'lucide-react';
...
export default function RoleBasedSidebar({ isOpen, closeSidebar }) {
  ...
  const instructorLinks = [
    ...
    { label: 'Upload Material', path: '/instructor/upload', icon: UploadCloud },
    { label: 'Earnings', path: '/instructor/earnings', icon: Wallet },
        ...
        {links.map((link) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) => 
                `flex items-center gap-3.5 px-4 py-3 rounded-lg text-xs font-bold tracking-wide transition-all duration-200 ${
                  isActive 
        ...
        {(role === 'instructor' || role === 'admin') && (
          <div className="pt-4 px-2">
            <Button 
              onClick={() => {
                closeSidebar();
                navigate(role === 'admin' ? '/admin/create-course' : '/instructor/create-course');
              }}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-xs font-bold text-gray-800 truncate max-w-[110px]">{user?.name || 'Kasun Perera'}</p>
